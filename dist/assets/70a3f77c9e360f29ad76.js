...
var DomController = function () {
  ...
  function getTotalPlannedTasks() {
    var plannedBtn = document.querySelector(PLANNED_BTN_CLASS);
    var plannedCard = plannedBtn.parentElement;
    var totalTasksList = plannedCard.getElementsByClassName(TOTAL_TASKS_CLASS);
    var matches = findAllDueToday('planned', 'Planned');
    if (totalTasksList.length > 0) {
      var totalTasks = totalTasksList[0];
      ...
      var _totalTasks = document.createElement(P);
      totalTasksWrapper.appendChild(_totalTasks);
      _totalTasks.classList.add(TOTAL_TASKS_CLASS);
      _totalTasks.innerHTML = matches.items.length > 99 ? NINETY_NINE_PLUS : matches.items.length;
    }
  }
  ;
  function getTotalDueTomorrowTasks() {
    var dueTomorrowBtn = document.querySelector(DUE_TOMORROW_BTN_CLASS);
    var dueTomorrowCard = dueTomorrowBtn.parentElement;
    var totalTasksList = dueTomorrowCard.getElementsByClassName(TOTAL_TASKS_CLASS);
    var matches = findAllDueToday(1, 'Due Tomorrow');
    if (totalTasksList.length > 0) {
      var totalTasks = totalTasksList[0];
      totalTasks.innerHTML = matches.items.length > 99 ? NINETY_NINE_PLUS : matches.items.length;
      ...
      totalTasksWrapper.classList.add(TOTAL_TASKS_WRAPPER_CLASS);
      var _totalTasks2 = document.createElement(P);
      totalTasksWrapper.appendChild(_totalTasks2);
      _totalTasks2.classList.add(TOTAL_TASKS_CLASS);
      _totalTasks2.innerHTML = matches.items.length > 99 ? NINETY_NINE_PLUS : matches.items.length;
    }
  }
  ;
  function getTotalFavouriteTasks() {
    var favouritesBtn = document.querySelector(FAVOURITES_BTN_CLASS);
    var favouritesCard = favouritesBtn.parentElement;
    var totalTasksList = favouritesCard.getElementsByClassName(TOTAL_TASKS_CLASS);
    var matches = findAllStarredTasks();
    if (totalTasksList.length > 0) {
      var totalTasks = totalTasksList[0];
      totalTasks.innerHTML = matches.items.length > 99 ? NINETY_NINE_PLUS : matches.items.length;
      ...
      totalTasksWrapper.classList.add(TOTAL_TASKS_WRAPPER_CLASS);
      var _totalTasks3 = document.createElement(P);
      totalTasksWrapper.appendChild(_totalTasks3);
      _totalTasks3.classList.add(TOTAL_TASKS_CLASS);
      _totalTasks3.innerHTML = matches.items.length > 99 ? NINETY_NINE_PLUS : matches.items.length;
    }
  }
  ;
  function getTotalOverdueTasks() {
    var overdueBtn = document.querySelector(OVERDUE_BTN_CLASS);
    var overdueCard = overdueBtn.parentElement;
    var totalTasksList = overdueCard.getElementsByClassName(TOTAL_TASKS_CLASS);
    var matches = findAllDueToday('pastdue', 'Past Due');
    if (totalTasksList.length > 0) {
      var totalTasks = totalTasksList[0];
      totalTasks.innerHTML = matches.items.length > 99 ? NINETY_NINE_PLUS : matches.items.length;
      ...
      totalTasksWrapper.classList.add(TOTAL_TASKS_WRAPPER_CLASS);
      var _totalTasks4 = document.createElement(P);
      totalTasksWrapper.appendChild(_totalTasks4);
      _totalTasks4.classList.add(TOTAL_TASKS_CLASS);
      _totalTasks4.innerHTML = matches.items.length > 99 ? NINETY_NINE_PLUS : matches.items.length;
    }
  }
  ;
  function getTotalDueTodayTasks() {
    var dueTodayBtn = document.querySelector(DUE_TODAY_CLASS);
    var dueTodayCard = dueTodayBtn.parentElement;
    var totalTasksList = dueTodayCard.getElementsByClassName(TOTAL_TASKS_CLASS);
    var matches = findAllDueToday(0, 'Due Today');
    if (totalTasksList.length > 0) {
      var totalTasks = totalTasksList[0];
      totalTasks.innerHTML = matches.items.length > 99 ? NINETY_NINE_PLUS : matches.items.length;
      ...
      totalTasksWrapper.classList.add(TOTAL_TASKS_WRAPPER_CLASS);
      var _totalTasks5 = document.createElement(P);
      totalTasksWrapper.appendChild(_totalTasks5);
      _totalTasks5.classList.add(TOTAL_TASKS_CLASS);
      _totalTasks5.innerHTML = matches.items.length > 99 ? NINETY_NINE_PLUS : matches.items.length;
    }
  }
  ;
  function displayLists() {
    ...
    function buildListCard(list) {
      ...
      var deleteBtnIcon = '<i id="task-delete" class="fa-solid fa-trash"></i>';
  ...
  function displayGoals(goalsList) {
    ...
    function onPriorityToggled(btnContainer, goal) {
      ...
      if (favouriteStatus) {
        var favouriteStar = '<i class="fa-solid fa-star"></i>';
        btnContainer.innerHTML = favouriteStar;
      } else {
        var notFavouriteStar = '<i class="fa-regular fa-star"></i>';
    ...
    function onCompletedToggled(btnContainer, goal) {
      ...
      if (goalCompleted) {
        var completedTask = '<i class="complete-task fa-solid fa-circle-check"></i>';
        btnContainer.innerHTML = completedTask;
      } else {
        var uncompletedTask = '<i class="incomplete-task fa-regular fa-circle"></i>';
